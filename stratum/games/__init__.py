...
def init_game_engine(engine_name, player_ids=[]):
    ...
    _CREATED_GAME_ID += 1
    players = [stratum.servers.client.remove_connected_client(pid) for pid in player_ids]
    engine_runner = stratum.engine.runner.init_engine_runner(_GAME_ENGINES[engine_name].Engine, engine_name, players)
